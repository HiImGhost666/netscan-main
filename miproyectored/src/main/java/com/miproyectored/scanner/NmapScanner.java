...
public class NmapScanner {
    ...
    private String findNmapPath() {
        ...
        if (os.contains("win")) {
            ...
            if (new java.io.File(commonPathProgramFiles).exists() && isNmapAvailable(commonPathProgramFiles))
                return commonPathProgramFiles;
            String commonPathProgramFilesX86 = "C:\\Program Files (x86)\\Nmap\\nmap.exe";
            if (new java.io.File(commonPathProgramFilesX86).exists() && isNmapAvailable(commonPathProgramFilesX86))
                return commonPathProgramFilesX86;
        } else { // Linux, macOS
            if (isNmapAvailable(command)) return command;
            // Podrías comprobar /usr/bin/nmap, /usr/local/bin/nmap, etc.
            String commonPathUsrBin = "/usr/bin/nmap";
            if (new java.io.File(commonPathUsrBin).exists() && isNmapAvailable(commonPathUsrBin))
                return commonPathUsrBin;
            String commonPathUsrLocalBin = "/usr/local/bin/nmap";
            if (new java.io.File(commonPathUsrLocalBin).exists() && isNmapAvailable(commonPathUsrLocalBin))
                return commonPathUsrLocalBin;
        }
        return null;
    }
                            ...
                            if (nmapPort.state != null && "open".equalsIgnoreCase(nmapPort.state.state)) {
                                try {
                                    ...
                                    if (nmapPort.service != null) {
                                        ...
                                        if (nmapPort.service.product != null)
                                            serviceDesc.append(" (").append(nmapPort.service.product);
                                        if (nmapPort.service.version != null)
                                            serviceDesc.append(" ").append(nmapPort.service.version);
                                        if (nmapPort.service.extrainfo != null)
                                            serviceDesc.append(" ").append(nmapPort.service.extrainfo);
